...
WHY THIS EXISTS:
  ...
  teams and six-figure software for this. Mid-market companies are stuck
  doing this in Excel.
...
# ── PLOTLY DARK BASE — used with update_layout(**DARK)
# Never re-declare xaxis/yaxis in individual update_layout calls;
# use update_xaxes / update_yaxes instead to avoid key collisions.
DARK = dict(
...
def build_ai_prompt(df: pd.DataFrame, tariffs: dict) -> tuple:
    ...
    top5_text = "\n".join([
        f"  • {r.product} ({r.country}): margin {r.margin_curr_pct:.1f}%→{r.margin_new_pct:.1f}% | "
        f"Annual loss: ${r.annual_margin_loss:,.0f} | Price hike needed: +{r.price_increase_pct:.1f}%"
    ...
    system = (
        ...
        "optimization. You provide precise, data-driven recommendations. You always cite specific "
        "numbers from the data. You are direct — no padding, no vague statements."
    )
...
""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div style="background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 14px;margin-bottom:16px;font-size:0.77rem;line-height:1.75;color:#8b949e">
...
Large companies have trade compliance teams and expensive software. Mid-market companies
are making multi-million dollar sourcing decisions in Excel spreadsheets.
<br><br>
...
my tariff drops but unit cost rises, lead time increases, I need extra safety stock,
and I face a 90-day PPAP re-qualification. What's my actual net saving?
</p>
</div>
""", unsafe_allow_html=True)
    with c2:
...
<br><br>
This is the analysis a supply chain consultant charges
<strong style="color:#3fb950">$50K to produce</strong>. TariffGuard generates it in seconds.
</p>
</div>
""", unsafe_allow_html=True)
    st.markdown("""
<div class="context-bar">
<strong style="color:#c9d1d9">Tariff context:</strong>
China +120pp · Vietnam +46pp · Mexico +25pp · Canada +25pp · Taiwan +32pp · India +26pp (2025 USTR).
The demo shows 3 China-sourced products going <strong style="color:#f85149">negative margin</strong> —
exactly what US importers are experiencing right now.
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    with cl:
        ...
        fig_bar.update_layout(
            **DARK,
            title=dict(text="Gross Margin: Current vs After Tariffs", font=dict(color="#f0f6fc", size=13)),
            barmode="group",
    ...
    with cr:
        ...
        fig_wf.update_layout(
            **DARK,
            title=dict(text="Annual Margin Erosion by Country", font=dict(color="#f0f6fc", size=13)),
            height=370,
            showlegend=False,
        )
    ...
    fig_bubble = px.scatter(
        df,
        x="margin_erosion", y="margin_new_pct",
    ...
    fig_bubble.update_layout(**DARK, height=400)
...
with tab2:
    ...
    rename = {
        "product":"Product","supplier":"Supplier","country":"Country","category":"Category",
...
TariffGuard includes the real decision costs: PPAP supplier qualification, bridge inventory carrying cost,
and production gap risk.
</div>
""", unsafe_allow_html=True)
    ...
    with st.expander("⚙️ Configure scenarios", expanded=True):
        ...
        with sp2:
            ...
            st.caption("Maintains margin %, risks customer volume loss")
        with sp3:
    ...
    avg_new_t        = df[mask]["new_tariff_pct"].mean() if n_high > 0 and df[mask]["new_tariff_pct"].mean() > 0 else 1
    ...
    breakeven_mo     = (total_transition / max(gross_saving / 12, 1.0)) if gross_saving > 0 else 999.0
    ...
    st.markdown(
        ...
        f'Switching <strong>{n_high}</strong> HIGH-risk product(s) to <strong>{alt_ctry}</strong> requires: '
        ...
        f'<strong>Total transition cost: ${total_transition:,.0f}</strong><br>'
    ...
    fig_sc.add_hline(y=0, line_dash="dash", line_color="#f85149", opacity=0.5,
                     annotation_text="Zero margin", annotation_font_color="#8b949e")
    fig_sc.update_layout(**DARK, height=400)
    fig_sc.update_xaxes(tickangle=-35)
